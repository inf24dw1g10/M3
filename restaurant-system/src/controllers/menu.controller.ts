...
import {Menu, ItemPedido} from '../models';
...
export class MenuController {
  ...
  // Endpoints adicionais para filtros específicos
  @get('/menu/categoria/{categoria}')
  @response(200, {
    description: 'Array of Menu items by category',
    content: {
      'application/json': {
        schema: {
          type: 'array',
          items: getModelSchemaRef(Menu),
        },
      },
    },
  })
  ...
  @get('/menu/disponiveis')
  @response(200, {
    description: 'Array of available Menu items',
    content: {
      'application/json': {
        schema: {
          type: 'array',
  ...
  @get('/menu/preco/{maxPreco}')
  @response(200, {
    description: 'Array of Menu items under specified price',
    content: {
      'application/json': {
        schema: {
          type: 'array',
  ...
  async findByPrecoMaximo(
    @param.path.number('maxPreco') maxPreco: number,
    @param.filter(Menu) filter?: Filter<Menu>,
  ): Promise<Menu[]> {
    return this.menuRepository.find({
      ...filter,
